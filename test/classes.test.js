...
import { matchClasses, classesOn, weekdayOf, shiftDate, validateClassChoice, windowFromEnv, UNSCHEDULED_CLASS, EARLY_MIN, LATE_MIN } from '../src/classes.js';
...
test('window edges: 3 hours before is in, 181 minutes is out; 3 hours after is in, 181 is out', () => {
  assert.equal(EARLY_MIN, 180);
  assert.equal(LATE_MIN, 180);
  // Tue Kids 3-5 at 16:00: window runs 13:00 to 19:00.
  assert.deepEqual(names(matchClasses(schedule, edt('2026-09-08T13:00'), TZ)), ['Kids 3-5@16:00']);
  assert.deepEqual(names(matchClasses(schedule, edt('2026-09-08T12:59'), TZ)), [], 'a minute too early');
  assert.ok(names(matchClasses(schedule, edt('2026-09-08T19:00'), TZ)).includes('Kids 3-5@16:00'), 'exactly 3 h after is in');
  assert.ok(!names(matchClasses(schedule, edt('2026-09-08T19:01'), TZ)).includes('Kids 3-5@16:00'), 'a minute too late');
});

test('windowFromEnv overrides the defaults, and ignores junk', () => {
  assert.deepEqual(windowFromEnv({}), { earlyMin: 180, lateMin: 180 });
  assert.deepEqual(windowFromEnv({ CHECKIN_EARLY_MIN: '45', CHECKIN_LATE_MIN: '15' }), { earlyMin: 45, lateMin: 15 });
  assert.deepEqual(windowFromEnv({ CHECKIN_EARLY_MIN: 30, CHECKIN_LATE_MIN: 0 }), { earlyMin: 30, lateMin: 0 });
  for (const bad of ['', 'soon', '-5', null, undefined]) {
    assert.deepEqual(windowFromEnv({ CHECKIN_EARLY_MIN: bad, CHECKIN_LATE_MIN: bad }), { earlyMin: 180, lateMin: 180 }, String(bad));
  }
  // A narrow window still works, which is how Johnny can tighten it later.
  const narrow = matchClasses(schedule, edt('2026-09-08T16:40'), TZ, { earlyMin: 45, lateMin: 15 });
  assert.deepEqual(names(narrow), ['Kids 6-9@16:30', 'Kids 10-14@17:15']);
});
...
test('back-to-back kids classes both match; program tag must disambiguate', () => {
  ...
  assert.deepEqual(names(r), ['Kids 3-5@16:00', 'Kids 6-9@16:30', 'Kids 10-14@17:15', 'Adult BJJ@18:15']);
  // The whole evening is in the window now, so the program tag is the only
  // thing that picks a class. Each program offers exactly one.
  const byProgram = (p) => r.matches.filter((m) => m.program === p).map((m) => m.name);
  assert.deepEqual(byProgram('kids-3-5'), ['Kids 3-5']);
  ...
  assert.equal(r.matches.find((m) => m.program === 'kids-6-9').startsInMin, -10);
  assert.equal(r.matches.find((m) => m.program === 'kids-10-14').startsInMin, 35);
});
...
test('no class right now', () => {
  ...
  assert.deepEqual(names(matchClasses(schedule, edt('2026-09-08T21:16'), TZ)), []); // after the last class's window
});

test('Wednesday is No-Gi for adults, kids keep their names', () => {
  const r = matchClasses(schedule, edt('2026-09-09T18:00'), TZ);
  assert.deepEqual(names(r), ['Kids 6-9@16:30', 'Kids 10-14@17:15', 'Adult No-Gi@18:15']);
  assert.equal(r.weekday, 'Wed');
  assert.deepEqual(r.matches.filter((m) => m.program === 'adult').map((m) => m.name), ['Adult No-Gi'], 'adults never see Adult BJJ on a Wednesday');
...
test('Saturday schedule', () => {
  ...
  assert.deepEqual(names(r), ['Kids 3-5@10:30', 'Kids 6-9@11:00', 'Kids 10-14@11:45', 'Adult BJJ@13:00']);
  assert.equal(r.nowLocal, '2026-09-05T10:50');
  assert.deepEqual(names(matchClasses(schedule, edt('2026-09-05T11:00'), TZ)), ['Kids 3-5@10:30', 'Kids 6-9@11:00', 'Kids 10-14@11:45', 'Adult BJJ@13:00']);
  // Early morning reaches only the first two.
  assert.deepEqual(names(matchClasses(schedule, edt('2026-09-05T08:30'), TZ)), ['Kids 3-5@10:30', 'Kids 6-9@11:00']);
  assert.equal(r.date, '2026-09-05');
  // Midday Saturday reaches the whole day's schedule.
  assert.deepEqual(names(matchClasses(schedule, edt('2026-09-05T12:30'), TZ)), ['Kids 3-5@10:30', 'Kids 6-9@11:00', 'Kids 10-14@11:45', 'Adult BJJ@13:00']);
});
...
test('DST: the week before and after fall-back both resolve 16:30 ET', () => {
  ...
  const tueEvening = ['Kids 3-5@16:00', 'Kids 6-9@16:30', 'Kids 10-14@17:15', 'Adult BJJ@18:15'];
  assert.deepEqual(names(matchClasses(schedule, edt('2026-10-27T16:30'), TZ)), tueEvening);
  assert.deepEqual(names(matchClasses(schedule, est('2026-11-03T16:30'), TZ)), tueEvening);
  ...
  assert.deepEqual(names(nov), tueEvening);
});
...
test('DST: spring-forward Sunday has no classes, the following Tuesday works', () => {
  ...
  assert.deepEqual(names(tue), ['Kids 3-5@16:00', 'Kids 6-9@16:30', 'Kids 10-14@17:15', 'Adult BJJ@18:15']);
});
