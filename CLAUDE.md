...
3. What the member receives at close-out, verified 2026-09-26 by a Claude in Chrome pass over the live account: **one email** at `executeAt` from the "Auto payment information" template ("$X is due on <date>", i.e. an upcoming auto-debit notice), then the receipt email after the charge. No SMS. The templates are GHL's built-in ones and cannot be edited here. The switches live at Payments → Invoices → Settings → Notifications → Customer Notifications and each applies to **every invoice in the account**; there is no per-schedule notification option in the UI. Receipts are a separate switch (Payments → Settings → Receipts) and are unaffected. Johnny's call (2026-09-26): members do not need the advance notice, since they authorized the charge and get a receipt. Turning off "Auto payment information" account-wide would also silence it for any other invoice with auto-pay (about 40 other invoices exist since July; which of them use auto-pay was not established). **Decision pending; the notice stays on until Johnny says otherwise.** Whether the schedule create body accepts a notification flag is unknown: this container cannot reach the GHL API docs.
4. Member agreement: a clause authorizing tab purchases to be charged to the card on file, saying they will get an email from BTT when their tab is about to be charged and a receipt after, with nothing to do.
