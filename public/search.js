...
// Plain ES module, no dependencies. displayName renders "First Last". Loaded by the pages with
 ...
 * "first last" prefix. Entries carry `first` and `last`.
...
export function search(roster, query, { min = MIN_CHARS, max = MAX_RESULTS } = {}) {
  ...
  for (const m of roster) {
    ...
    const lastKey = normalize(m.last || '');
    let hit = false;
    if (parts.length === 1) {
      hit = first.startsWith(q) || (lastKey.length > 0 && startsWithKey(lastKey, q));
    } else {
      // Whole phrase as a first-name prefix ("mary ann") or a last-name
      // prefix ("da silva"), or split at any word boundary into
      // first-name prefix + last-name prefix.
      hit = first.startsWith(q) || startsWithKey(lastKey, q);
...
/** "Jack Silva", or just "Jack" when there is no last name. */
